...
auto main(void) -> int
{
    try
    {
        ...
        {
            ZxFile ofs{ "123.txt", ZxFile::OpenMod::WriteNew };
            if (ofs)
            {
                ofs << std::uint32_t(12);
        ...
        ZxFile ofs{ "123.txt", ZxFile::OpenMod::ReadExists };
...

        {
            ...
            ZxFile ofs_u8{ u8"乱七八糟的文件名.txt", ZxFile::OpenMod::WriteNew };
            ofs_u8 << std::span{ std::string_view("乱七八糟的内容") };
            ofs_u8.Close();

            ZxFile ifs_u8{ u8"乱七八糟的文件名.txt", ZxFile::OpenMod::ReadExists };
        ...
        assert(noexcept_open.OpenNoThrow("1290u012nf812", ZxFile::OpenMod::ReadExists) == false);
