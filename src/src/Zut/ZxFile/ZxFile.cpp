...
namespace ZQF::Zut
{
  ...
  auto ZxFile::Open(const std::string_view msPath, const OpenMod eMode) -> void
  {
    if (this->IsOpen())
    {
      throw std::runtime_error(std::string{ "ZxFile::Open(): already holds a file handle -> msPath: " }.append(msPath));
    }
    const auto file_id_opt = ZxFilePlat::Open(msPath, eMode);
    if (file_id_opt.has_value() == false)
    {
      throw std::runtime_error(std::string{ "ZxFile::Open(): open file failed! -> msPath: " }.append(msPath));
    }
    m_hFile = *file_id_opt;
  }
